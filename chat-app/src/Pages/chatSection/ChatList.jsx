...
const ChatList=({ contacts })=>{
   ...
   //  const {user}=useUserStore();
    ...
    if (!user) {
        return (
            <div className={`flex items-center justify-center h-screen ${theme === "dark"
...
{filteredContacts.map(contact => (
    ...
    className={`p-3 flex items-center cursor-pointer ${theme === "dark" ? 
        ...
        <img src={contact.avatar.secure_url} alt={contact.username} className="h-12 w-12 rounded-full "/>
...
<div className="flex justify-between items-baseline">
    <p className={`text-sm ${theme === "dark" ? "text-gray-400" : "text-gray-500"} truncate`}>
        {contact?.conversation?.lastMessage?.content || "No messages yet"}
                    ...
                    {contact?.conversation?.lastMessage?.receiver === user?._id && contact?.conversation?.unreadCount > 0 && (
